...
[
  {
    ...
    "region": string?,                // регион/штат, если упомянут
    "city": string?,                  // город, если упомянут
    "travelAngle": string,            // под каким углом это интересно туристу
    "seasonality": string?,           // напр. "весна и осень" — только если явно сказано
    "targetAudience": string?,        // напр. "семьи, авторские туры"
    "confidenceScore": number,        // 0..1, насколько точно это travel-инфоповод
    "reasonWhyRelevant": string?      // одно предложение, почему этот инфоповод выбран
  }
]
